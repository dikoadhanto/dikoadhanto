<template>
  <div class="card">
    <b-row class="head-wrapper">
      <b-col>
        <h2 class="mt-7 ml-10">Update PORT</h2>
      </b-col>
      <b-col>
                ...
                <form @submit.prevent="" class="ml-25 mr-25">
                  ...
                  <div class="d-flex justify-content-between border-top pt-10">
                    <div class="d-flex justify-content-start">
                      <!-- <button class="btn btn-sm btn-success mr-5">
                        SIMPAN & KEMBALI
                      </button> -->
                    </div>
...
export default {
  ...
  methods: {
    ...
    updatePort() {
      ...
      if (this.name == "" || this.name == null) {
        ...
        Swal.fire({
          icon: "error",
          title: "Oops..",
          text: "Data Rotation tidak boleh kosong",
        });
      } else {
        this.inputs.map((items) => {
        ...
        if (this.is_validate) {
          ...
          ApiService.patch(
            `${process.env.VUE_APP_ROOT_API_4}/port/update/${this.idSelected}`,
            dataAddRtu
          )
            .then(() => {
              this.inputs = [];
    ...
    updateConfig() {
      ApiService.post(
        `${process.env.VUE_APP_ROOT_API_4}/port/config/${this.idSelected}`
      )
        .then(({ data }) => {
          ...
          } else if (data.message.includes("The config is being updated")) {
            Swal.fire({
              icon: "warning",
              title: "Tunggu",
              text: `${data.message}`,
    ...
    getDataport() {
      ApiService.get(
        ...
        .then(({ data }) => {
          ...
          // this.selectMdiviceId = data.result.device_id;
          this.calibration_value = data.result.calibration_value;
          this.data_rotation = data.result.data_rotation;
    ...
    getOperation(rule, flag) {
      if (rule != null) {
        if (flag === "logika") {
          ...
          if (flag === "atas") {
            return tresholds[0].replace(/\s/g, "");
          } else if (flag === "bawah") {
            return tresholds[1].replace(/\s/g, "");
          }
        }
      } else {
        return "";
    ...
    deleteForm(index) {
      Swal.fire({
        title: `Yakin ingin menghapus treshold ${this.inputs[index].label}?`,
        icon: "danger",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: "Ya, hapus treshold!",
      }).then((result) => {
        if (result.isConfirmed) {
          if (this.inputs[index].id_m_threshold != "") {
            ApiService.delete(
              `${process.env.VUE_APP_ROOT_API_4}/port/del/treshold/${this.inputs[index].id_m_threshold}`
            );
          }
          Swal.fire("Terhapus!", "Data treshold berhasil dihapus.", "success");
