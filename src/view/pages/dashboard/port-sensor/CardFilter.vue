...
export default {
  ...
  watch: {
    getIdRegionalSensor: function () {
      if (
        this.getIdRegionalSensor == null ||
        this.getIdRegionalSensor.length == 0
      ) {
        this.getAllDataWitel();
        this.getAllDataDatel();
        this.getAllDataLokasi();
    ...
    getIdWitelSensor: function () {
      if (this.getIdWitelSensor == null || this.getIdWitelSensor.length == 0) {
        if (
          this.getIdRegionalSensor == null ||
          this.getIdRegionalSensor.length == 0
        ) {
    ...
    getIdDatelSensor: function () {
      if (this.getIdDatelSensor == null || this.getIdDatelSensor.length == 0) {
        if (
          this.getIdWitelSensor == null ||
          this.getIdWitelSensor.length == 0
        ) {
          if (
            this.getIdRegionalSensor == null ||
            this.getIdRegionalSensor.length == 0
  ...
  methods: {
    async setData() {
      this.filter.regional = this.getIdRegionalSensor;
      if (this.filter.regional != null) {
        await this.getDataWitel();
      }
      this.filter.witel = this.getIdWitelSensor;
      if (this.filter.witel != null) {
        await this.getDataDatel();
      }
      this.filter.datel = this.getIdDatelSensor;
      if (this.filter.datel != null) {
        await this.getDataLocation();
      }
      this.filter.location = this.getIdLocationSensor;
      this.filter.tags = this.getIdTagsSensor;
      this.filter.alarm = this.getIdAlarmTypeSensor;
    },
    setDefaultLocation() {
      this.$store.dispatch("setLocationSensor", null);
    ...
    setDataRegional() {
      this.$store.dispatch("setRegionalSensor", this.filter.regional);
    ...
    setDataWitel() {
      this.$store.dispatch("setWitelSensor", this.filter.witel);
      this.filter.datel = null;
      this.dataDatel = [];
      this.filter.location = null;
      this.dataLokasi = [];
    ...
    setDataDatel() {
      this.$store.dispatch("setDatelSensor", this.filter.datel);
      this.filter.location = null;
      this.dataLokasi = [];
      this.changeCheckLocation();
    ...
    setDataLocation() {
      this.$store.dispatch("setLocationSensor", this.filter.location);
    },
    setDataTags() {
      this.$store.dispatch("setTagsSensor", this.filter.tags);
    },
    setDataAlarm() {
      this.$store.dispatch("setAlarmTypeSensor", this.filter.alarm);
    },
  },
};
</script>
