  ...
  methods: {
    async changeNewCategory(category, filter, dateFormat) {
      ...
      try {
        ...
        data.result.forEach((element) => {
          ...
          this.valueChart.push(element.value_avg.toFixed(2));
        ...
        console.log(error.response, "error digital.. .. ..");
        if (this.isReady) {
          this.$refs["chart"].updateOptions(this.chartOptions);
          this.$refs["chart"].updateSeries(this.series);
        }
    ...
    async getDataRealtime() {
      ...
      try {
        ...
        const dataRealtime = data.result.data.result[0].values;
    ...
    setFormatTglStepDate(data, format) {
      if (data == null || data == "") {
        ...
        return textHelper.formatDate(format, data);
      }
    },
  },
};
