  ...
  <section>
    <div class="table-responsive">
      <table class="table table-bordered table-striped" v-if="isReady">
        <TableHeader />
        <tbody>
          <tr v-for="(genset, i) in dataGenset" :key="i">
            <td v-if="currentPage == 1">{{ i + 1 }}</td>
            <td v-else>{{ i + 1 + (currentPage - 1) * perPage }}</td>
            <td>{{ genset.no_port }}</td>
            <td>{{ genset.identifier }}</td>
            <td>{{ genset.description }}</td>
            <td>{{ genset.metrics_value }}</td>
            <td>{{ genset.units }}</td>
          </tr>
        </tbody>
      </table>
  ...
  methods: {
    getdataGenset() {
      ...
      ApiService.get(
        ...
        `operation/rtu/topology/${this.idRtu}/?page=${this.currentPage}&limit=${this.perPage}&mode=digital&desc=GENSET`
      )
        .then(({ data }) => {
          ...
          this.dataGenset = data.result.payload;
          this.isEmpty = false;
          this.isReady = true;
        })
        .catch((e) => {
