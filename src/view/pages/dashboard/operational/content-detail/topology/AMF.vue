  ...
  <section>
    <div class="table-responsive">
      <table class="table table-bordered table-striped" v-if="isReady">
        <TableHeader />
        <tbody>
          <tr v-for="(amf, i) in dataAmf" :key="i">
            <td v-if="currentPage == 1">{{ i + 1 }}</td>
            <td v-else>{{ i + 1 + (currentPage - 1) * perPage }}</td>
            <td>{{ amf.no_port }}</td>
            <td>{{ amf.identifier }}</td>
            <td>{{ amf.description }}</td>
            <td>{{ amf.metrics_value }}</td>
            <td>{{ amf.units }}</td>
          </tr>
        </tbody>
      </table>
  ...
  methods: {
    getDataAmf() {
      ...
      ApiService.get(
        ...
        `operation/rtu/topology/${this.idRtu}/?page=${this.currentPage}&limit=${this.perPage}&mode=digital&desc=AMF`
      )
        .then(({ data }) => {
          ...
          this.dataAmf = data.result.payload;
          this.isEmpty = false;
          this.isReady = true;
        })
        .catch((e) => {
